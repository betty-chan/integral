...
class GradeController extends Controller {
    ...
    public async page() {
        const { ctx } = this
        let result = await ctx.service.grade.findGrade()
    ...
    public async editPower() {
        const { ctx } = this
        const garde = ctx.request.body
        await ctx.service.grade.addPower(garde)
        ctx.returnBody(200, "操作成功")
    }
    //删除
    public async deletePower() {
        const { ctx } = this
        const query = ctx.request.query
        let result = null;
        if (query.id) {
            result = await ctx.service.grade.deletePower({ id: query.id })
        }
        ctx.returnBody(200, "操作成功", result)
    }
}
