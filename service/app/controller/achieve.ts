...
class AchieveController extends Controller {
    ...
    //列表查询
    public async pageMedal() {
        const { ctx } = this
        const page = ctx.request.query;
        let result = null;
        if (page) {
            result = await ctx.service.achieve.findMedal(page)
        }
        ctx.returnBody(200, "操作成功", result)
    }
    //列表添加或修改
    public async editMedal() {
        const { ctx } = this
        const garde = ctx.request.body;
        await ctx.service.achieve.addMedal(garde)
        ctx.returnBody(200, "操作成功")
    }
    //删除
    public async deleteMedal() {
        const { ctx } = this
        const query = ctx.request.query
        let result = null;
        if (query.id) {
            result = await ctx.service.achieve.deleteMedal({ id: query.id })
        }
        ctx.returnBody(200, "操作成功", result)
    }
    //列表添加或修改
