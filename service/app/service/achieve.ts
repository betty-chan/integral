import { Service } from 'egg';
import { achieveParams, medalParams } from './type/sroce-interface'

function flattenObj(values) {
    let keys1 = Object.keys(values);
    keys1.forEach((key) => {
        let name = key.split(".");
        let item = ""
        key.split(".").length && (item = name[name.length - 1]);
        if (!values[item]) {
            values[item] = values[key];
            delete values[key]
        }
    });
    return values;
}
export default class achieveService extends Service {
    /*
     * 分页查询
     */
    public async findAchieve(query, page) {
        let { ctx } = this
        ctx.model.TMedal.belongsTo(ctx.model.TAchieve, {
            foreignKey: "id",
            targetKey: "medal_id",
            constraints: false,
        })
        let result = await ctx.model.TMedal.findAndCountAll({
            offset: page.pageNumber - 1,
            limit: +page.pageSize,
            raw: true,//表明返回的数据是json而不是model
            include: [
                {
                    model: ctx.model.TAchieve,
                    attributes: ['user_id'],
                    where: query,
                    required: false
                }
            ],
            order: ['id']
        })
        result.rows.forEach((element) => {
            flattenObj(element)
        });
        return result;
    }
    public async findRecord(page) {
        let { ctx } = this
        ctx.model.TAchieve.belongsTo(ctx.model.TUser, {
            foreignKey: "user_id",
            targetKey: "id",
            constraints: false,
        })
        let result = await ctx.model.TAchieve.findAndCountAll({
            offset: page.pageNumber - 1,
            limit: +page.pageSize,
            raw: true,//表明返回的数据是json而不是model
            include: [
                {
                    model: ctx.model.TUser,
                    attributes: ['username'],
                }
            ],
        })
        result.rows.forEach((element) => {
            flattenObj(element)
        });
        return result;
    }
    /*
    * 分页查询勋章
    */
    public async findMedal(page) {
        let { ctx } = this
        return await ctx.model.TMedal.findAndCountAll({
            offset: page.pageNumber - 1,
            limit: +page.pageSize,
        })
    }
    /*
     * 添加或者更新
     * @interface {sroceUserParams}
     */
    public async addMedal(obj: medalParams) {
        let { ctx } = this
        if (obj.id) {
            return await ctx.model.TMedal.update(obj, {
                where: {
                    id: obj.id
                }
            });
        } else {
            return await ctx.model.TMedal.create(obj);
        }
    }
    /*
    * 删除
    */
    public async deleteMedal(query) {
        let { ctx } = this
        return await ctx.model.TMedal.destroy({ where: query });
    }
    /*
     * 添加或者更新
     */
    public async addAchieve(obj: achieveParams) {
        let { ctx } = this
        if (obj.id) {
            return await ctx.model.TAchieve.update(obj, {
                where: {
                    id: obj.id
                }
            });
        } else {
            return await ctx.model.TAchieve.create(obj);
        }
    }
    /*
     * 删除
     */
    public async deleteAchieve(query) {
        let { ctx } = this
        return await ctx.model.TAchieve.destroy({ where: query });
    }
}
